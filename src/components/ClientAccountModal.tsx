...
export default function ClientAccountModal({
  ...
  useEffect(() => {
    ...
    let localCreds: Record<string, { password?: string }> = {};
    try {
      localCreds = JSON.parse(localStorage.getItem("local_profiles_credentials") || "{}");
    } catch {
      localCreds = {};
    }

    const existingCred = localCreds[client.email?.toLowerCase().trim()];
    setPassword(existingCred?.password || client.password || "");
    setShowPassword(false);
    setCopied(false);
  }, [client?.id, client?.email, client?.password, isOpen]);
          ...
          <div className="p-3 bg-neutral-900/50 border border-neutral-850 rounded-xl flex items-center justify-between gap-3">
            ...
            <div className="flex items-center gap-2">
              <button
                ...
                onClick={() => {
                  if (client.phone) {
                    const msg = getClientAccountWhatsAppTemplate({
                      clientName: client.name,
                      email: client.email,
                      password
                    });
                    openWhatsAppMessage(client.phone, msg);
